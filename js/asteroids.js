...
async function load() {
  ...
  try {
    ...
    const url = `https://api.nasa.gov/neo/rest/v1/feed?start_date=${start.toISOString().slice(0,10)}&end_date=${end.toISOString().slice(0,10)}&api_key=${NASA_KEY}`;
    const data = await fetchJSON(url);
    const ast = flatten(data.near_earth_objects || {});
    const nearest = ast[0];
    ...
    app.innerHTML = `<section class="card" style="padding:1rem;"><canvas id="orbit" width="1000" height="320" style="width:100%;height:320px"></canvas><div id="tooltip" class="mono" style="position:absolute;display:none;background:#031521;padding:.35rem .5rem;border:1px solid var(--border-rest);border-radius:6px;"></div></section>
    ...
    <section class="stats-row" style="margin-top:1rem;">
      <div class="card stat-card"><div class="stat-label">TOTAL TRACKED THIS WEEK</div><div class="stat-number" id="s1">0</div></div>
      <div class="card stat-card"><div class="stat-label">POTENTIALLY HAZARDOUS</div><div class="stat-number" id="s2">0</div></div>
      <div class="card stat-card"><div class="stat-label">CLOSEST APPROACH (km)</div><div class="stat-number" id="s3">0</div></div>
      <div class="card stat-card"><div class="stat-label">FASTEST OBJECT (km/s)</div><div class="stat-number" id="s4">0</div></div>
    </section>
    ...
    ast.forEach((a) => {
      ...
      tr.onclick = () => {
        detail.innerHTML = `<div class='card' style='padding:1rem'><h3 class='section-title'>${a.name}</h3><pre style='white-space:pre-wrap;'>${JSON.stringify(a.raw, null, 2)}</pre></div>`;
      };
    ...
    canvas.onmousemove = (e) => {
      ...
      if (hit) {
        tip.style.display = 'block'; tip.style.left = `${e.clientX + 8}px`; tip.style.top = `${e.clientY + 8}px`; tip.textContent = hit.name;
