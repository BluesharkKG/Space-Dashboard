...
async function load() {
  renderSkeleton(app, 5);
  try {
    const eventsData = await cachedFetch('https://eonet.gsfc.nasa.gov/api/v3/events/geojson?status=open', 10);
    const categoriesData = await cachedFetch('https://eonet.gsfc.nasa.gov/api/v3/categories', 10);
    const features = eventsData.features || [];
    const categories = categoriesData.categories || [];

    app.innerHTML = `<section class='stats-row'><div class='card stat-card'><div class='stat-label'>TOTAL OPEN EVENTS</div><div id='s1' class='stat-number'>0</div></div><div class='card stat-card'><div class='stat-label'>WILDFIRES</div><div id='s2' class='stat-number'>0</div></div><div class='card stat-card'><div class='stat-label'>STORMS</div><div id='s3' class='stat-number'>0</div></div><div class='card stat-card'><div class='stat-label'>VOLCANOES</div><div id='s4' class='stat-number'>0</div></div></section>
    ...
    try {
      ...
      const render = () => {
        layer.clearLayers();
        list.innerHTML = '';
        features.filter((f) => !active || (f.properties.categories || []).some((c) => c.title === active)).forEach((f) => {
      ...
      document.getElementById('map').innerHTML = `<div class='error-card'>MAP UNAVAILABLE<br><small>${mapErr.message}</small></div>`;
    }
