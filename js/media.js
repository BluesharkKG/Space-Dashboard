...
function card(item) {
  ...
  return `<button class='card media-card' data-id='${d.nasa_id}' style='padding:0;overflow:hidden;text-align:left;'>
    <img src='${thumb}' alt='${d.title || ''}' style='width:100%;height:180px;object-fit:cover'>
...
async function search(reset=false) {
  ...
  try {
    ...
    if (reset) { cache = []; grid.innerHTML = ''; }
    cache.push(...items);
    cache.forEach((it) => {
      if (grid.querySelector(`[data-id="${it.data?.[0]?.nasa_id}"]`)) return;
      grid.insertAdjacentHTML('beforeend', card(it));
    });
  } catch (e) {
    showError(document.getElementById('grid'), e.message, () => search(reset));
  }
}
...
function openModal(id) {
  ...
  modal.innerHTML = `<div style='background:#091322;border:1px solid var(--border-rest);max-width:900px;width:min(92vw,900px);max-height:88vh;overflow:auto;padding:1rem;border-radius:12px'>
    ${mediaType === 'video' ? `<video src='${link}' controls style='width:100%;max-height:60vh'></video>` : `<img src='${link}' style='width:100%;max-height:60vh;object-fit:contain'>`}
